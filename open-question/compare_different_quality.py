...
def train_unet(model, model_name, save_to="", noise_ratio=0):
    ...
    for epoch in range(epochs):
        ...
        print(
            f"[{model_name}] Epoch {epoch+1} | TrainLoss={train_loss/len(train_loader):.4f} | ValLoss={val_loss:.4f} | ValIoU={val_iou:.4f}"
        )
...
def evaluate_model(unet, test_loader, device):
    ...
    with torch.no_grad():
        for x, _, image_ids, _ in test_loader:
            ...
            # CAM original and noisy versions
            ...
            cam_iou_5 += (
                (
                    (cam_noise_5 * trimap).sum((1, 2, 3))
                    / ((cam_noise_5 + trimap).clamp(0, 1).sum((1, 2, 3)) + 1e-6)
                )
                .sum()
                .item()
            )
            ...
            cam_iou_10 += (
                (
                    (cam_noise_10 * trimap).sum((1, 2, 3))
                    / ((cam_noise_10 + trimap).clamp(0, 1).sum((1, 2, 3)) + 1e-6)
                )
                .sum()
                .item()
            )
...
if __name__ == "__main__":
    ...
    # 5% corrupted pseudo pixels
    if not os.path.exists("models/unet_dirty_5.pth"):
        train_unet(
            dirty_5_model,
            model_name="unet_dirty_5.pth",
            save_to="models/unet_dirty_5.pth",
            noise_ratio=0.05,
        )
    else:
        dirty_5_model.load_state_dict(
            torch.load("models/unet_dirty_5.pth", map_location=torch.device("cpu"))
        )

    # 10% corrupted pseudo pixels
    if not os.path.exists("models/unet_dirty_10.pth"):
        train_unet(
            dirty_10_model,
            model_name="unet_dirty_10.pth",
            save_to="models/unet_dirty_10.pth",
            noise_ratio=0.1,
        )
    else:
        dirty_10_model.load_state_dict(
            torch.load("models/unet_dirty_10.pth", map_location=torch.device("cpu"))
        )
    ...
    if os.path.exists("models/best_unet_dirty_5.pth"):
        best_5_model.load_state_dict(
            torch.load("models/best_unet_dirty_5.pth", map_location=device)
        )
        best_5_model.eval()
        print(
            "Dirty model trained on 5% noisy pseudo label evaluated on clean labels):"
        )
    ...
    if os.path.exists("models/best_unet_dirty_10.pth"):
        best_10_model.load_state_dict(
            torch.load("models/best_unet_dirty_10.pth", map_location=device)
        )
        best_10_model.eval()
        print(
            "Dirty model trained on 10% noisy pseudo label evaluated on clean labels):"
        )
